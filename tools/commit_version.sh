...
LANE="${2:-manual}"
SRC="$ROOT/extracted/$VER"; REPO="$ROOT/repo"; LOCK="$ROOT/tmp/commit.lock"
emit(){ python3 "$ROOT/tools/emit_status.py" "$LANE" "$VER" "$@"; }
[ -d "$SRC" ] || { emit commit 0 '{}' "extracted tree missing: $SRC"; exit 1; }
[ -f "$SRC/MANIFEST.json" ] || { emit commit 0 '{}' "MANIFEST.json missing in $SRC (run tree_manifest.py first)"; exit 1; }
