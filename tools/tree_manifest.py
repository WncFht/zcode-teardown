#!/usr/bin/env python3
"""Generate extracted/<ver>/MANIFEST.json: full tree listing + engine/tool/host fingerprints.
...
def tool_version(path):
    try:
        r = subprocess.run([path, '--version'], capture_output=True, text=True,
                           timeout=15, env={**os.environ, 'DISPLAY': ''})
...
def main():
    ...
    native = [{"p": f["p"], "s": f["s"], "h": f["h"]}
              for f in files if f["p"].endswith('.node')]

    appmeta = {}
    pkg = os.path.join(out, 'app', 'package.json')
    if os.path.isfile(pkg):
        try:
            pj = json.load(open(pkg))
            appmeta = {"name": pj.get('name'), "version": pj.get('version'),
                       "main": pj.get('main'),
    ...
    subprocess.run([sys.executable, os.path.join(ROOT, 'tools', 'emit_status.py'),
                    lane, ver, 'manifest', '1', json.dumps(man['counts'])])
