...
\begin{takeaway}
55 个版本分三个 era：v2.3.0 首秀为诚实的 opt-in，v2.6.0 默认翻转为 opt-out（界面仍显示关），v3.1.0 起开关被移出管线成为纯装饰——此后是否上传只由登录态、workspace 类型与服务端 credential 签发决定。
\end{takeaway}
...
\begin{takeaway}
55 个版本 UI 零披露；诊断导出刻意排除快照目录；ARMS 遥测只见 credential 端点 host+path。日志如实记录但用户不可见——拨动装饰开关本身被遥测。
\end{takeaway}
...
\begin{takeaway}
披露报告核心指控对 v3.x 成立；v2.3.0–v2.5.0 不成立（当时是诚实的 opt-in）；v2.6.0–v2.13.0 半成立。若干技术细节与语料不符。
\end{takeaway}
...
\begin{takeaway}
语料为 55 个版本的 extracted 树快照，逐版 tag 经 bare git 比对；v2.x 代码未压缩可直接读，v3.x 为 minified 单行需经 \cid{i(fn,"name")} 注解还原。全部断言可在语料中复核。
\end{takeaway}

语料为 ZCode linux-x64 全部 55 个入库版本（v2.2.0–v3.12.3；3.7.1/.2/.4 三版发布后从 CDN 撤下、按 published-but-unrecoverable 记账，版本边界用相邻可恢复版桥接）。每个版本的安装包经解包后以 \cid{v<tag>} 形式快照在语料 git 仓中。

分析分两层：其一是对 55 个 tag 做 28 项指示符的全量矩阵，用于定位功能首秀与行为边界；其二是按主题对 bundle 做精读还原，v2.x 代码未压缩、v3.x 为 minified 单行（原始标识符经 \cid{s(fn,"name")}、\cid{i(fn,"name")}、\cid{a(fn,"name")} 注解恢复）。所有版本边界均按真实 tag 列表逐一核实——不存在的 tag（v3.2.0/v3.6.0/v3.7.0/v3.8.0/v3.9.0/v3.11.0）不计入零命中。
