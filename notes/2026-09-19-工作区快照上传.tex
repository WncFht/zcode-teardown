...
\newcommand{\cid}{%
  ...
  \codeSpan}
\newcommand{\codeSpan}[1]{{\small\texttt{#1}}\endgroup}
...
\newenvironment{takeaway}{%
  \begin{mdframed}[backgroundcolor=black!4,linecolor=black!20,
    linewidth=0.5pt,skipabove=7pt,skipbelow=7pt,
    innertopmargin=5pt,innerbottommargin=5pt,
    innerleftmargin=8pt,innerrightmargin=8pt]
  \small\textbf{要点}\quad\ignorespaces
}{\end{mdframed}}

\title{ZCode 工作区快照上传机制\\——基于 56 个发布版本的逆向分析}
\author{}
...
功能与设置开关的关系经历了三个阶段。v2.3.0–v2.5.0 期间开关真实门控、默认关闭，须用户主动开启。v2.6.0 起默认翻转为开启：只要用户没有显式关过，管线就在运行，而设置界面仍按原始开关值显示为关闭。v3.1.0 起开关被从捕获管线整体移除，是否上传只由登录态、本地工作区类型与服务端是否签发凭证决定——唯一的实质同意门从此位于服务端。在存活期的全部 55 个版本中，产品界面从未告知这一行为；设置页唯一相关的文案把它描述为一个纯本地的“索引”功能。\textbf{v3.14.0（2026-09-19）该功能族被整体移除}：上传管线、credential 端点、两个设置开关、全部索引区 UI 文案、repo-wiki 本地功能本体、glm/zcode.cjs 两枚死导出全部删除，28 个功能指示符归零且无继任端点——同意门问题以“功能不存在”告终（见 \ref{sec:removal} 节）。本报告主体描述 v2.3.0–v3.12.3 存活期行为。

主要结论如下。

\begin{enumerate}
\item 功能自 v2.3.0 首秀，存活区间为 v2.3.0–v3.12.3，实现始终位于 \cid{app/out/host/index.js}；其他 bundle 中的同名符号均为共享 schema 常量与设置键，不存在第二份实现。v3.14.0 整体移除。
...
\item repo-wiki 是纯本地功能，与上传管线没有数据通路，快照仅搭用它的触发器时机；v3.14.0 与管线同版移除，说明它在产品中始终与快照绑定。
\end{enumerate}
...
\begin{takeaway}
管线五步：取 credential（GET）→ 枚举过滤文件 → tar+gzip → AES-256-CTR 加密 + RSA 包装密钥 → multipart POST 到 OSS → OSS 回传 callback 完成登记。credential 响应先于磁盘扫描，因为打包依赖其中的 \cid{snapshot_id} 与 \cid{public_key}。
\end{takeaway}
...
\begin{takeaway}
56 个版本分四个 era：v2.3.0 首秀为诚实的 opt-in，v2.6.0 默认翻转为 opt-out（界面仍显示关），v3.1.0 起开关被移出管线成为纯装饰——此后是否上传只由登录态、workspace 类型与服务端 credential 签发决定；v3.14.0 整体移除。
\end{takeaway}

\subsection{首秀判定与实现载体}

v2.2.0 时 \cid{upload-credential}、\allowbreak\cid{captureBeforePrompt}、\allowbreak\cid{repoSnapshot}、\allowbreak\cid{repo-snapshot} 四个指示符全部零命中；同版中 \cid{checkpoints}、\cid{snapshot}、\cid{aliyuncs}、\cid{aes-256-ctr} 的命中经逐一核对均为无关噪音——\cid{checkpoints} 是已有的 GitCheckpointStore 本地回滚功能，\cid{aliyuncs} 是 Qwen provider 端点 \cid{dashscope.aliyuncs.com}，\cid{snapshot} 命中全是 git-diff 快照类型。v2.3.0 首秀即完整形态：\cid{app/out/host/index.js} 内嵌 \cid{repo-snapshot/*.ts} 的 esbuild 源文件 banner，可读出十个源模块（artifact、hasher、canonicalJson、paths、filter、scanner、\allowbreak sidecarService、stateRepo、credentialDiagnostics、uploadClient、uploadWorker）。
...
\caption{功能边界演化：横轴为 56 个入库 tag 按版本排序，颜色区分主题；v3.14.0 处为整体移除边界。}
\label{fig:timeline}
\end{figure}
...
v3.14.0 & \textbf{整体移除}：sidecar/captureBeforePrompt/credential GET/tar/AES/RSA/OSS PostObject/callback/pending/state/GC/配额/intent 调度器全清零（feature 指示符 28→0）；\cid{/api/v1/snapshot/upload-credential} 端点消失且无继任；repo-wiki 本地功能本体同版移除；\cid{repoSnapshotIndexingEnabled}/\allowbreak\cid{optimizeAgentExperienceEnabled} 设置键与 \cid{settings.indexing} 全系 UI 文案删除；glm/zcode.cjs 死导出删除；scheduler bundle 2.1MB→1.0MB。残留仅卫生字面量：诊断导出排除表 \cid{_L} 仍列历史目录名，ARMS 路径段白名单仍含 \cid{snapshot}/\cid{upload-credential} 段（\ref{sec:removal} 节）。 \\
\end{longtable}
...
\begin{takeaway}
\cid{git ls-files} 列出的一切进包——v3.1.0 起含完整 \cid{.git/} 树（remote URL token、reflog、pack 文件全收）且豁免后续全部过滤；v3.11.1 起追加全局配置与 prompt 附件。无文件数或总字节上限，secret 过滤覆盖很窄。
\end{takeaway}
...
v2.x 对 \cid{.git} 有三重排除：ls-files 不列、filter 段规则、walkFiles 剪枝。v3.1.0 起这一策略翻转，全部 41 个 3.x tag 行为一致：\cid{appendRootGitMetadata}\allowbreak\cid{Paths}\allowbreak 对 \cid{<workspace>/.git} 做 \cid{lstat}——ENOENT 则不动；是目录则由 \cid{walkGitMetadataFiles}\allowbreak 零剪枝递归追加每一个文件（objects/pack/refs/logs/config/hooks/index/LFS、\cid{modules/<sub>/…}、\cid{worktrees/} 全收）；是文件（linked worktree 的 gitfile）则只打指针本身；是符号链接则静默贡献零（\cid{lstat} 不跟随）。过滤链中 \cid{.git} 段紧随 symlink 检查返回 include，豁免 secret、大小、二进制、dep-cache-build 全部后续过滤；嵌套的 \cid{.git/} 目录不枚举，只认根。后果是 \cid{.git/config} 里的 remote URL token、reflog、数 MB 的 pack 文件全量外发。

\subsection{extraManifest（v3.11.1 上线）}

\cid{repo_snapshot_}\allowbreak\cid{extra_manifest} 字面量在语料中 grep 不到，因为 schema 名由 \cid{repo_snapshot_}\allowbreak\cid{<name>/<ver>} 模板拼出；边界为 v3.10.2 到 v3.11.1。extra 内容打进同一个加密 tar：
...
\item 组 \cid{global-configs}（\cid{changePolicy:"rare"}）：\allowbreak\cid{settings.behavior.json}\allowbreak 按 17 键白名单打包（含两个快照开关自身的取值，另剥离 model/\allowbreak provider/\allowbreak baseURL/\allowbreak language/\allowbreak theme/\allowbreak locale/\allowbreak window/\allowbreak layout/\allowbreak zoom/\allowbreak workspacePath 等键）、\cid{mcp.json}（MCP 服务器配置逐字）、\cid{skills.json}、\cid{commands.json}、\cid{hooks.json}（shell hook 命令串一并上传）、\cid{memory.json}（20MiB 截断）、\cid{subagents.json}、\cid{plugins.json}、\cid{instructions.json}（\cid{~/.zcode/AGENTS.md}，$\leq$20MiB）。\cid{sanitizeUnknown} 把键名匹配 \verb+api_key|access_token|+\allowbreak\verb+refresh_token|secret|+\allowbreak\verb+password|credential|+\allowbreak\verb+authorization|cookie|+\allowbreak\verb+session_token|token+ 的值改写为 \cid{<redacted>}。
...
引入后唯一变化是 v3.12.1 把 subagents 的 \cid{thoughtLevel} 字段来源改为\allowbreak \cid{modelSelection.}\allowbreak\cid{options.reasoningLevel}。
...
\begin{takeaway}
tar 根目录名即服务端签发的 \cid{snapshot_id}；增量判据只看路径与文件大小，等长改写不可见。state.json 历经七个结构 era 追踪 pending 上传；pending 唯一投递机会是捕获尾部 flush，无启动或周期 flush。
\end{takeaway}
...
\cid{manifestHash} 由以下规范化 JSON 的 sha256 得出；canonical JSON 的规则是对象键排序、\cid{undefined} 丢弃、数组保序：

\begin{code}
sha256.hex(canonicalJson({
    schema:"repo_snapshot_manifest_hash/v1",
    workspaceKey, files:[{path,sizeBytes}] sorted }))
\end{code}
...
prompt.json：v2.x 的 schema 为 \cid{repo_snapshot_prompt/v1}，字段含 \cid{taskId}、\cid{traceId}、\cid{createdAt}、\cid{manifestHash}、\cid{content}；v3.1.0 起（/v2 名自 v3.1.1）：
...
uploadKey（\cid{repo_snapshot_upload_key/v1}，从未升版）：

\begin{code}
{ schema, snapshotId, keyId,
  keyWrapAlgorithm: "rsa-oaep-sha256", publicKeySpkiPem }
\end{code}

v3.1.0 起加 \cid{baseSnapshotId?}；v3.2.3 起加 \cid{maxSizeBytes?}；v3.6.1 起加 \cid{uploadCredentialHandle}。

\cid{repo_snapshot_upload_target/v1} 是不上线的内部对象：

\begin{code}
{ schema, workspaceKeyHash, kind, manifestHash, baseManifestHash,
  encryptedArtifact: { …envelope, encryptedSizeBytes, encryptedSha256 } }
\end{code}

v3.6.1 起附 \cid{uploadCredentialHandle} 与 \cid{attribution}。

\subsection{baseline 与 increment 决策}
\label{sec:delta}

v2.x 是纯本地决策：\cid{lastAccepted}\allowbreak\cid{ManifestHash}\allowbreak 与 \cid{lastAccepted}\allowbreak\cid{ManifestPath}\allowbreak 齐备、且 \cid{readAccepted}\allowbreak\cid{BaseManifest}\allowbreak 复算\allowbreak manifestHash 通过，则发 \cid{kind=}\allowbreak\cid{"increment"}，否则 \cid{baseline}。v3.1.0 起加了服务端门：credential 必须同时下发 \cid{snapshot.}\allowbreak\cid{base_snapshot_id} 才允许 increment，本地可验 base 仍是必要条件。v3.11.1 起 extra-manifest 有同构的平行门。

需要强调的是，差分基的选择权在客户端：\cid{buildRepoSnapshot}\allowbreak\cid{Delta}\allowbreak 消费的是本地 \cid{lastAccepted}\allowbreak\cid{Manifest}（落盘在状态目录的那一份），服务端下发的 \cid{base_snapshot_id} 只被回显进 \cid{x:base_snapshot_id} 归因与 uploadKey 记录。服务端能靠拒发它强制客户端下次走 full，但无法指认客户端用哪份 manifest 做差分。

服务端拒收的通道在客户端不存在：OSS callback 响应体从不解析（\cid{uploadObject} 只看 \cid{response.ok}），为拒收准备的 \cid{base_not_found}/\allowbreak\cid{base_invalid}/\allowbreak\cid{hash_mismatch} 三个处理分支自 v2.3.0 写好后从未可达。客户端实际只产生四种 reason：\cid{invalid}（缺 \cid{snapshot_id} 或无 credential）、\cid{key_expired}（v3.6.1 起）、\allowbreak\cid{payload_too_large}（v3.2.1 起）、\allowbreak\cid{object_upload_failed}。服务端真拒收时客户端无感知，照常 \cid{markAcceptedManifest}——这是协议层的记账盲点。

\subsection{state.json 与 pending 文件}

状态根目录为 \cid{<config>/repo-snapshots/}（v2.3.0–v3.1.3），v3.2.0 起迁至 \cid{<config>/checkpoints/}（\cid{migrateLegacyRepoSnapshot}\allowbreak\cid{RootDir}\allowbreak 每进程执行一次 \cid{renameSync}，与 GitCheckpointStore 共用\allowbreak \cid{checkpoints/<hash12>/}——workspaceHash 同为\allowbreak \cid{sha256(workspaceKey)[:12]}）。子目录有 \cid{manifests}、\cid{pending}、\cid{tmp} 与 \cid{state.json}，v3.11.1 起加 \cid{extra-manifests/}。pending 文件名即 groupId：v3.2.0 起为 \cid{<manifestHash>.<createdAt>}，v3.11.1 起中段插入 extraManifestHash。

\cid{state.json} 历经七个结构 era。基线字段为 \cid{pendingUpload}（字段含 \cid{kind}、\allowbreak\cid{encryptedArtifactPath}、\allowbreak\cid{encryptionEnvelopePath}、\allowbreak\cid{manifestPath}、\allowbreak\cid{baseManifestHash?}、\allowbreak\cid{nextManifestHash}、\allowbreak\cid{createdAt}）加 \cid{lastAcceptedManifestHash} 与\allowbreak \cid{lastAcceptedManifestPath}：
...
C2 & v3.2.3 & \verb|lastCompressedSize{encryptedSizeBytes,|\allowbreak\verb|workspaceSizeBytes,manifestHash,recordedAt}| \\
...
\begin{takeaway}
AES-256-CTR 加密 + RSA-OAEP-SHA256 包装密钥，经 OSS PostObject 直传；callback 字段承载 wrapped key 与归因但响应体从不解析——服务端拒收对客户端完全不可见。\cid{data:null} 是唯一的实质同意门。
\end{takeaway}
...
非 2xx 时 \cid{readApiJson} 抛 \cid{ApiError}，message 依次取 body 的 \cid{error|message|}\allowbreak\cid{detail|msg}，同时捕获 \cid{x-request-id}、\cid{x-trace-id}、\cid{x-span-id} 诊断头。v3.12.x 的 apiClient 走 undici transport（honor \cid{httpProxy}/\cid{noProxy}/\cid{httpProxyCaCertPath}），401 时触发 \cid{onZcodeJwtInvalid} 钩子。

响应校验 \cid{resolveUploadCredential}\allowbreak\cid{Data}：envelope \verb|{code,msg,data}|，\cid{code!==0} 抛错，\cid{!data} 返回 null——这就是服务端的 kill switch。必填字段：\verb+callback.{url,body,+\allowbreak\verb+content_type}+、\allowbreak\verb+oss.{host,path,policy,+\allowbreak\verb+x_oss_*}+、\allowbreak\verb+encryption.{public_key,+\allowbreak\verb+key_version,algorithm}+、\allowbreak\cid{snapshot.}\allowbreak\cid{snapshot_id}，缺失抛”missing fields”并把各子对象键名写进本地日志。可选：\cid{data.max_size}（v3.2.1 起解析、规范化、非法剥离）、\allowbreak\cid{data.snapshot.}\allowbreak\cid{base_snapshot_id}（v2.13.0 起）；\cid{ttl}、\cid{expires} 类字段从不读取。

credential 复用有两代。v2.3.0–v3.5.3：\cid{uploadCredentialsBy}\allowbreak\cid{CacheKey} 以 \verb|sha256(token)[:16]|\allowbreak\verb|:workspaceId| 为键，\cid{requestUploadTarget} 取出即删除。v3.6.1 起：\cid{uploadCredentialsBy}\allowbreak\cid{Handle} 以 UUID 为键、记录 tokenHash/workspaceId/1h 过期点，一次性消费，\cid{workspaceId} 或 \cid{tokenHash} 不符即 \cid{key_expired}；pending 落盘只写 handle 不写 credential 本体，重启后内存 map 为空、必然 \cid{key_expired}——这是设计行为而非缺陷。

\subsection{OSS PostObject 上传}

\cid{buildObjectUpload}\allowbreak\cid{Target}：v3.2.1 起先比 \cid{encryptedSizeBytes}\allowbreak\cid{ > }\allowbreak\cid{data.max_size}，超限记 \cid{payload_too_}\allowbreak\cid{large}；缺 \cid{snapshot_id} 记 \cid{invalid}。成功返回 \verb|{method:"POST",|\allowbreak\verb| url:oss.host,|\allowbreak\verb| formFields:{…}}|，表单字段按插入序为 \cid{success_action_}\allowbreak\cid{status}、\allowbreak\cid{policy}、\allowbreak\cid{x-oss-signature} 系列、\allowbreak\cid{key}、\allowbreak\cid{x-oss-security}\allowbreak\cid{-token}、\allowbreak v3.1.0 起的裸归因字段、\allowbreak 最后是 \cid{callback}；\allowbreak\cid{file} 由 \cid{uploadPost}\allowbreak\cid{Object} 以\allowbreak \cid{repo-snapshot.}\allowbreak\cid{tar.gz.enc} 文件名追加。\cid{expiresAt}、\allowbreak\cid{maxBytes}、\allowbreak\cid{objectKey}、\allowbreak\cid{snapshotId}、\allowbreak\verb+callback:{mode:+\allowbreak\verb+"oss-callback"}+ 是无消费方的死元数据。
...
单文件大于 1MiB 在扫描期即弃（\cid{large-file}，全版本）；扫描无总数上限。产物上限有三处：\cid{data.max_size}（v3.2.1 起，加密后比对，超限 \cid{payload_too_large}）、\allowbreak\cid{uploadKey.maxSizeBytes-16}\allowbreak（v3.2.3 起，gzip 输出逐条断言，超限抛 \cid{RepoSnapshotArtifactMaxSizeExceededError}）、v3.11.1 起的磁盘配额（驻留量约束，名义上限约 6GiB）。超限一律静默跳过。失败 reason 与去向见 \ref{sec:delta} 节：客户端产出 \cid{invalid}/\allowbreak\cid{key_expired}/\allowbreak\cid{payload_too_large}/\allowbreak\cid{object_upload_failed} 四种，\cid{base_not_found}/\allowbreak\cid{base_invalid}/\allowbreak\cid{hash_mismatch} 三种为死代码。
...
\begin{takeaway}
v2.x 仅 prompt 发送触发；v3.x 增至五类直接触发（prompt、steer、V4 命令、任务终止、wiki 生成）与两类间接注入（cron、OffPeak）。无 timer/启动/文件监视触发。
\end{takeaway}
\label{sec:triggers}

本节描述存活期（v2.3.0–v3.12.3）行为；v3.14.0 全部触发器随 sidecar 移除（\ref{sec:removal} 节）。
...
\item 远程 relay：\cid{createRemoteRepoWiki}\allowbreak\cid{ServiceRelay}\allowbreak 暴露 \cid{captureTaskComplete}\allowbreak\cid{Update}\allowbreak 与 \cid{refreshExisting}\allowbreak\cid{WikiAfter}\allowbreak\cid{TaskComplete}\allowbreak 两个 thunk，是 v3.4.0–v3.5.3 间任务终止触发的唯一调用方。
\end{itemize}
...
\begin{takeaway}
三阶段演化：opt-in → 静默翻转的 opt-out → 纯装饰。v3.1.0 起开关不门控任何行为，唯一写入方是 renderer 开关与 \cid{normalizeSettings}\allowbreak\cid{Patch}，不存在自动开启路径；实质同意门只剩服务端 credential 签发。
\end{takeaway}
...
阶段一（v2.3.0–v2.5.0），诚实的 opt-in。唯一读取点是 \cid{indexingEnabledProvider} 判\allowbreak \cid{settings.repoSnapshot}\allowbreak\cid{IndexingEnabled===true}（zod 默认 false），位于 \cid{captureBeforePromptUnsafe} 首行，门控捕获及下游全部上传；UI 开关绑原始 flag，显示与行为一致。

阶段二（v2.6.0–v2.13.0），默认翻转的 opt-out。provider 改调\allowbreak \cid{isRepoSnapshotIndexing}\allowbreak\cid{EffectivelyEnabled}\allowbreak（八个版本逐字一致）：
...
出厂默认（\cid{enabled=false}、\cid{userConfigured} 未设）下管线运行；只有显式 opt-out（\allowbreak\cid{enabled===false}\allowbreak\cid{ && }\allowbreak\cid{userConfigured}\allowbreak\cid{===true}）才停。\cid{normalizeSettings}\allowbreak\cid{Patch}\allowbreak 会在任何布尔写入路径上强置 \cid{userConfigured}\allowbreak\cid{=true}；renderer 的开关仍绑原始 flag——显示 OFF 时管线在跑，用户拨过一次之后显示与行为才一致。

阶段三（v3.1.0–v3.12.3），开关装饰化。\cid{RepoSnapshotSidecarService}\allowbreak 类体内对两个 flag 零引用（逐版枚举确认）。\cid{repoSnapshotIndexingEnabled}\allowbreak 的全部”消费点”只剩 schema 声明、\allowbreak\cid{normalizeSettingsPatch}\allowbreak 记账、设置页显示（v3.x 绑 \cid{enabled && userConfigured} 复合谓词——被静默开启的 flag 显示 OFF）以及 v3.11.1 起的 global-configs 白名单（取值被打进快照 payload）。捕获路径的实际前置条件全是运行时态：登录 token 存在、\cid{workspaceIdentity} 为空（remote workspace 永不捕获）、credential 非 null（\cid{data:null} 即中止——全版本仅存的实质同意门）、v3.11.1 起磁盘配额、产物不超过 maxSize。写入方审计显示，两个 flag 的唯一写入方是 renderer 开关与 \cid{normalizeSettingsPatch}，不存在任何自动开启路径。

阶段四（v3.14.0），开关随功能一并删除。\cid{repoSnapshotIndexingEnabled}/\allowbreak\cid{userConfigured}/\allowbreak\cid{optimizeAgentExperienceEnabled} 三个设置键及其 schema 声明、\cid{settings.indexing}/\allowbreak\cid{settings.optimizeAgentExperience} 全系 UI 字典、renderer 开关组件全部归零（rg 0 命中）——同意门问题以“功能不存在”告终，而非以“门被修好”告终（\ref{sec:removal} 节）。

\subsection{optimizeAgentExperienceEnabled：死配置}

v3.2.0 首秀时 zod 默认 true，v3.2.5 翻为默认 false 并加迁移标记。全部 55 个存活版中该键没有任何功能消费点，只出现在 schema、迁移逻辑与隐私页开关文案（“Allow us to use your conversations to improve the Agent experience. We protect your data privacy and security.”）里——是死配置或占位；v3.14.0 键随功能删除。
...
\begin{takeaway}
repo-wiki 是纯本地功能，不消费快照结果；快照仅搭车其触发链。设置页”索引””instant grep””All data is stored locally”等文案均无本地实现——所谓索引就是这次上传。
\end{takeaway}

repo-wiki 是纯本地功能，已全链核实：存储为 \cid{repo-wiki/}\allowbreak\cid{<sha256(workspaceKey)[:12]>/} 下的本地 JSON；生成走用户自己配置的 LLM provider（向用户 baseURL 拼 \cid{/chat/completions} 等端点，带 \cid{repo_wiki_*} querySource 标签）；删除只删本地文件；全语料没有任何 wiki REST 端点；wiki 代码从不读取 \cid{snapshot_id}/\cid{base_snapshot_id}。

快照与 wiki 的关系是搭车：\cid{captureRepoWikiSnapshot}\allowbreak 只是对同一个 \cid{repoSnapshotSidecar.}\allowbreak\cid{captureBeforePrompt}\allowbreak 的包装，wiki 不等上传、不消费结果，服务端拿到的只是 \cid{“repo-wiki-generation”}/\allowbreak\cid{“repo-wiki-update”} 哨兵串加 meta。删除确认框中”OSS repository upload records are not affected”指的就是本管线的上传记录。

所谓”索引”本体就是这次上传：全语料对 \cid{trigram}、\cid{buildIndex}、\cid{indexFiles}、\cid{grepIndex}、\allowbreak\cid{instantGrep*} 零命中（ripgrep/ugrep 命中是远程 workspace 的资源包）；\cid{instantGrepIndexingEnabled}\allowbreak 与 \cid{repoSnapshotIndexingEnabled}\allowbreak 均无本地功能消费点——“Index new folders”“instant grep”“All data is stored locally”这些文案没有任何本地实现支撑（可能服务端索引由上传驱动，文案与实现不符，见 \ref{sec:open} 节）。wiki 自己的 manifestHash 是 \cid{getWorkspaceOverview()} 本地扫描的陈旧性判据，与快照的 manifestHash 同名不同源。

v3.14.0 起本节整体成为历史：repo-wiki 本地功能本体（wikiService、\cid{captureTaskCompleteUpdate}、\allowbreak\cid{generate*}、\allowbreak\cid{LocalWorkspaceRepoReader}、\allowbreak\cid{repo_wiki} 存储路径）与快照管线同版移除——\cid{repo-wiki} 字面量仅残留在诊断导出排除表 \cid{_L} 的目录名单里（\ref{sec:removal} 节）。

\section{披露、日志与遥测}
\label{sec:disclosure}

\begin{takeaway}
存活期 55 个版本 UI 零披露；诊断导出刻意排除快照目录；ARMS 遥测只见 credential 端点 host+path。日志如实记录但用户不可见——拨动装饰开关本身被遥测。v3.14.0 起连“未披露”的对象本身都不存在了。
\end{takeaway}

结论是存活期全部 55 个版本的 UI 零披露：没有 label、toast、dialog、tooltip、onboarding consent、隐私声明、EULA 或设置描述告知工作区被快照加密上传。逐版 consent 面审计中唯一沾边的文案均不相关（conversationShare 警告、feedback 主动上传、Chrome-profile adminConsent、支付 ToS、插件 privacyPolicy）。
...
证据锚：era B 字典键 \cid{settings.indexing.repoTitle}/\allowbreak\cid{settings.indexing.beta}/\allowbreak\cid{settings.indexing.}\allowbreak\cid{repoDescription}（v2.3.0 与 v2.13.0 的 renderer bundle；zh-CN 字典里 repoTitle 为英文原文，非误植）；era C 字典与组件在 v3.1.0 与 v3.12.3 的 renderer 资源中；”OSS 仓库上传记录/OSS repository upload records”字面量仅见于\allowbreak \cid{repoWiki.deleteConfirm}\allowbreak\cid{Description}；era D 文案在 \verb+settings.optimizeAgent+\allowbreak\verb+Experience{,Description}+（v3.10.0 起带 analytics envelope）。era E（v3.14.0）：上述全部字典键、组件与文案 rg 0 命中——不是改文案而是整段删除。
...
日志面如实记录、与 UI 的沉默形成对照：v2.x 有 \cid{info "indexing accepted"}，v3.1.x 有\allowbreak \cid{info "repo snapshot upload accepted"}\allowbreak（v3.2.0 起删除）与含 userId 的 \cid{debug "...prompt meta prepared"}；logger 命名空间为 \cid{repoSnapshotSidecar}/\allowbreak\cid{repoSnapshotUploadClient}/\allowbreak\cid{repoSnapshotUploadWorker}，落 \cid{~/.zcode/cli/log/}\allowbreak\cid{zcode-<date>.jsonl} 与 host 结构化日志。

诊断导出刻意排除快照目录：排除表覆盖 \cid{agent-config}、\cid{certs}、\cid{repo-snapshots}、\cid{repo-wiki}、\cid{sessions}、\cid{session-bindings}、\cid{checkpoints} 七项（\cid{credentials.json} 另按名排除），导出的 zip 为纯本地文件。用户经诊断导出既看不到也发不出快照状态。feedback 上传走另一条 OSS 管线（边界串 \cid{----zcode-feedback-}、端点 \cid{/feedback/attachment/upload-credential}），勿与快照 uploader 混淆。
...
\begin{takeaway}
披露报告核心指控对 v3.x 成立；v2.3.0–v2.5.0 不成立（当时是诚实的 opt-in）；v2.6.0–v2.13.0 半成立。若干技术细节与语料不符。裁决对象为存活期 v2.3.0–v3.12.3；v3.14.0 起机制不存在，各条不再适用（\ref{sec:removal} 节）。
\end{takeaway}
...
无任何用户披露 & \textbf{成立}（存活期全部 55 版 UI 零披露；本地日志如实记录“upload accepted”但不对用户展示） \\
\end{longtable}

\section{能力边界}

\begin{takeaway}
服务端持 RSA 私钥可解全部明文；OSS 见密文与裸归因但无法解密；客户端无法得知服务端是否接受上传；用户无任何客户端侧关闭手段——唯一旁路是退出登录或迁往 remote workspace。
\end{takeaway}
\label{sec:boundary}
...
v3.14.0 起上述客户端侧机制全部不存在：无代码可抓 credential、无 artifact 可落盘、无开关可拨；服务端是否保留历史已上传数据、是否另开通道，语料不可判（\ref{sec:open} 节未决项）。

\section{v3.14.0：整体移除与遗留面}
\label{sec:removal}

\begin{takeaway}
v3.14.0 把整个功能族连根移除——不只是上传管线，而是“快照 + 索引 + repo-wiki”一整族；28 个功能指示符归零，无继任端点。残留只有卫生字面量（诊断排除表目录名、ARMS 路径段白名单）与独立的 GitCheckpointStore/feedback 管线。移除动机语料外不可判。
\end{takeaway}

v3.14.0（2026-09-19 发布，距 v3.12.3 仅 3 天）的移除逐项盘点如下，全部经 \cid{extracted/3.14.0/app/} 全树 rg 核实，28 指标矩阵见 \cid{notes/snapshot-tracking.json}。

\textbf{端点}：\cid{/api/v1/snapshot/upload-credential} 字面量 0 命中；全树 \verb+/api/v*/snapshot*+ 0 命中，无继任端点（新版仅新增 \verb+/api/v1/marketing/touch{,/action}+ 等无关端点）。\cid{/api/v1/event/report} 为唯一同域残留通用端点。

\textbf{管线代码}：\cid{captureBeforePrompt}/\allowbreak\cid{repoSnapshot*}（67→0）/\allowbreak\cid{PostObject}/\allowbreak AES-256-CTR（仅剩 ssh2 node\_module 命中）/\allowbreak\cid{rsa-oaep}/\allowbreak\cid{publicKeySpkiPem}/\allowbreak\cid{keyWrapAlgorithm}/\allowbreak\cid{key_version}/\allowbreak\cid{extraManifest}/\allowbreak\cid{extra_manifest}/\allowbreak\cid{snapshot_manifest}/\allowbreak\cid{lastCompressedSize}/\allowbreak\cid{isRepoSnapshotIndexing}\allowbreak\cid{EffectivelyEnabled} 全部 0 命中——扫描、过滤、tar、加密、pending、state.json 七 era、GC、磁盘配额、intent 调度器无一残留。

\textbf{触发器}：T1–T5 随 \cid{RepoSnapshotSidecarService} 整体消失（reserve/activate/schedule callsite 0 命中）；\cid{sendConversationCommandV4} 包络本体保留但快照占槽代码删除。cron/OffPeakRun 的 automation 引擎保留（那是独立功能，曾只是注入器）。

\textbf{设置与 UI}：\cid{repoSnapshotIndexingEnabled}/\allowbreak\cid{repoSnapshotIndexingUserConfigured}/\allowbreak\cid{optimizeAgentExperienceEnabled} 三个键及 schema 声明 0 命中；\verb+settings.indexing.*+/\allowbreak\verb+settings.optimizeAgentExperience*+/\allowbreak instantGrep/“All data is stored locally”/“OSS repository upload records”/“Improve experience” 全部 UI 文案 0 命中（存活期同类文案合计 435 命中→1）；renderer 索引区组件与隐私开关消失。

\textbf{repo-wiki 本体}：wikiService/\allowbreak\cid{captureTaskCompleteUpdate}/\allowbreak\cid{generate*}/\allowbreak\cid{refreshExistingWiki}/\allowbreak\cid{LocalWorkspaceRepoReader}/\allowbreak\cid{repo_wiki} 存储路径 0 命中——这个“纯本地”功能与上传管线同版移除，说明它在产品里始终与快照绑定存在。

\textbf{glm/zcode.cjs 死导出}：\cid{isRepoSnapshotIndexingSwitchChecked}/\allowbreak\cid{resolveOptimizeAgentExperienceEnabled} 删除——v3.12.2 才加的 staged API surface 未接线即夭折。

\textbf{体积}：\cid{app/out/scheduler/index.js} 2,108,514B→1,000,385B（−53\%），automations cron 引擎保留。

\textbf{残留（卫生字面量，无功能）}：诊断导出排除表 \verb+_L=["agent-config","certs","repo-snapshots","repo-wiki","sessions","session-bindings","checkpoints"]+ 仍列历史目录名；ARMS 遥测路径段白名单仍含 \cid{snapshot}/\cid{upload-credential}/\cid{workspace-bridge} 段；\cid{GitCheckpointStore} 本地撤销功能保留并继续使用 \cid{checkpoints/} 目录——老安装遗留的 \cid{state.json}/pending \cid{.enc} 与其共址，未见针对遗留快照文件的清理代码（孤儿 \cid{.enc} 原地残留，不再被读也不再被传）；feedback 附件 OSS 管线完整保留（\cid{/feedback/attachment/upload-credential} + x-oss 表单 + \verb+----zcode-feedback-+ 边界——用户主动发起的另一条管线，勿混淆）。

\textbf{边界与动机}：语料内移除边界为 v3.12.3→v3.14.0；3.13.x 在 linux-x64 不存在（mac/win 同路径模式亦 404，不排除他平台存在过该线）。移除动机在语料外不可判——是披露响应、产品下线还是服务端侧迁移，二进制无证据；能确认的只有“客户端侧机制已不存在”。

\section{未决问题与已知边界行为}
\label{sec:open}

\begin{takeaway}
\cid{workspaceIdentity} 精确格式、v2.x 诊断导出行为等若干 REVIEW 项待查。已确认缺陷：等长改写对增量不可见、空工作区照常上传、自扫窗口、失败无日志等。
\end{takeaway}
...
\item v3.14.0 移除的若干不可判项：动机（披露响应/产品决策/服务端迁移）语料外不可判；服务端是否保留历史上传数据、是否另开客户端外通道不可判；\cid{~/.zcode/v2/checkpoints/} 老遗留 state/pending 无清理代码但亦不再被读；3.13.x 若在他平台存在过则移除边界可能更早，语料内按 v3.14.0 记。
\end{itemize}
...
\begin{takeaway}
语料为 56 个版本的 extracted 树快照，逐版 tag 经 bare git 比对；v2.x 代码未压缩可直接读，v3.x 为 minified 单行需经 \cid{i(fn,"name")} 注解还原。全部断言可在语料中复核。
\end{takeaway}

语料为 ZCode linux-x64 全部 56 个入库版本（v2.2.0–v3.14.0；3.7.1/.2/.4 三版发布后从 CDN 撤下、按 published-but-unrecoverable 记账，版本边界用相邻可恢复版桥接；3.13.x 在 linux-x64 语料中不存在，manifest 从 3.12.3 直跳 3.14.0）。每个版本的安装包经解包后以 \cid{v<tag>} 形式快照在语料 git 仓中。

分析分两层：其一是对 56 个 tag 做 28 项指示符的全量矩阵，用于定位功能首秀与行为边界；其二是按主题对 bundle 做精读还原，v2.x 代码未压缩、v3.x 为 minified 单行（原始标识符经 \cid{s(fn,"name")}、\cid{i(fn,"name")}、\cid{a(fn,"name")} 注解恢复）。所有版本边界均按真实 tag 列表逐一核实——不存在的 tag（v3.2.0/v3.6.0/v3.7.0/v3.8.0/v3.9.0/v3.11.0）不计入零命中。
...
文中插图由 matplotlib 脚本生成，SVG 与 PDF 双份落在 notes/assets/。
