...
Scope: v3.12.3（build 7463）→ v3.14.0（build 7681，build-meta commit `a1328db1`、buildTime `2026-09-19T01:46Z`、electron-builder 26.8.1）。**3.13.x 整条线未发布**——releases/{3.13.0..3.13.4}/linux-x64/_.deb HEAD 全 404，3.12.4/3.12.5 亦无；版本号从 3.12.3（09-16）直跳 3.14.0（09-19 03:29Z 上传 CDN，距 buildTime ~2h）。官方 `latest.yml`/`latest-linux.yml` 静态归档已于 2026-09-19 前后 CDN 级撤除（全部版本含已存档者均 404），宣称/哈希的官方渠道转为应用内更新 manifest（`zcode.z.ai/api/v1/releases/electron/manifest?platform=<plat>`，只服务最新版，双语 releaseNotes + 全平台 sha512——其 3.14.0 deb sha512 与本仓自算值一致）；官网 changelog 已列 3.14.0。`manifest/versions.json` 的 size/releaseDate 取自 deb HEAD（`last-modified`）。deb 162,038,244 B（+15.1MB vs 3.12.3）。官方宣称对照：动态工作流（=CreateWorkflow/createWorkflow_ 落地）、办公/编程双模式切换（呼应 `ZCODE_PRODUCT_FLAVOR`）、快捷键录制生效（呼应 `zcode:set-shortcut-recording-active` IPC）、三步新手引导、归档任务批量删除——与二进制面互证。
