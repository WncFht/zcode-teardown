...
- ~~**3.12.3 endpoint `-61` is an extraction artifact, not a product change**~~ resolved: all signatures regenerated 2026-09-18 evening with the final junk-host-filtered extractor; 3.12.2 and 3.12.3 now both yield 283 endpoints (identical counts all categories) and `diffs/3.12.2__3.12.3.md` is all-zero at signature level — true delta is only the updater fix above. (Original evidence: `_perf.extract_s=0.67` vs 1.32, generated 15:50 before 3.12.2's 16:28 = ran over a partially-extracted tree.)
...
- **app-update.yml `localhost:8081`** present in ALL my versions (`extracted/<ver>/app-update.yml`) — an inert stub: real feed is computed in-app (domestic/overseas CDN + `ZCODE_UPDATE_FEED_URL` override). NOT a 3.12.3 packaging defect. Corpus-wide re-check: yml is `cdn.zcode-ai.com` ≤3.3.5 and `localhost:8081` ≥3.3.6 (all 25 later trees uniform) — the stub-ification boundary is **3.3.5→3.3.6** (c3 lane), landing with the in-app `/api/v1/releases/electron/manifest` feed + update-UI IPC surface that same version.
