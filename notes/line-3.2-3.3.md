...
- **`app-update.yml` feed stub-ified at this exact boundary**: `url:` flips `cdn.zcode-ai.com` (≤3.3.5, all 21 earlier trees) → `http://localhost:8081` (≥3.3.6, all 25 later trees uniform through 3.12.3). Not a QA leak — it lands together with the new in-app `/api/v1/releases/electron/manifest` feed + update-UI IPC surface above, i.e. electron-updater's generic yml feed was deliberately dead-ended as update checking moved in-app (`ZCODE_UPDATE_FEED_URL`/`ZCODE_AUTO_UPDATE_DEV*` override path).
...
5. **Update pipeline rebuilt in stages**: `ZCODE_UPDATE_FEED_URL`/`_RELEASE_ROOT_URL` (3.3.2) → custom `/api/v1/releases/electron/manifest` yaml feed + full update-UI IPC surface (3.3.6), with `TEST_UPDATER_ARCH`/`ZCODE_AUTO_UPDATE_DEV*` testability hooks. The static `app-update.yml` feed is `cdn.zcode-ai.com/…/update/linux/x64/` ≤3.3.5 and `localhost:8081` ≥3.3.6 — stub-ified at the same boundary the in-app manifest feed lands (see boundary table above).
...
- ~~**Signatures for 3.2.0, 3.2.1, 3.2.4, 3.3.2 are gen-new** while neighbors are gen-old/gen-mid~~ resolved: all 55 signatures regenerated 2026-09-18 evening with the final junk-host-filtered extractor — single generation corpus-wide; verified the filter only drops vendored-doc/spec URLs (chevrotain.io, wikipedia, whatwg, ietf, unicode.org, stackoverflow, example.com) and `plan:lesson_plan`, zero real endpoints lost (3.12.2 A/B: 358→283, gained=0). My same-generation comparisons above remain valid as historical analysis.
- ~~**`app-update.yml` `url: http://localhost:8081` in 3.3.6 deb**~~ resolved: uniform across all ≥3.3.6 trees (25 versions); it's the stub left when the in-app `/api/v1/releases/electron/manifest` feed superseded the yml — the REVIEW worry ("dev feed leaked") is ruled out by corpus-wide uniformity.
